 ...
 function viewadpro(){
    ...
    if($tblad3){
        ...
        if(mysql_num_rows($tblad3)>0){
            ...
            echo"<table border='1px solid' style='width:100%;margin:auto;text-align:center;background-color:aliceblue'>";
            echo"<tr><th>ID</th><th>Tên hạn mức</th><th>tên hạng mục</th><th>số tiền chi</th><th>số tiền cảnh báo</th><th>số tiền hạn mức</th><th>thời gian bắt đầu</th><th>thời gian kết thúc</th><th>Tên tài khoản</th><th>Action</th></tr>";
            while($row= mysql_fetch_assoc($tblad3)){
               
                echo"<tr><td>".$row["id"]."</td><td>".$row["tenhanmuc"]."</td><td>".$row["tenhangmuc"]."</td><td>".$row["tongchi"]."</td><td>".$row["sotiencanhbao"]."</td><td>".$row["sotienhanmuc"]."</td><td>".$row["thoigianbatdau"]."</td><td>".$row["thoigianketthuc"]."</td><td>".$row["tenTaiKhoan"]."</td><td><button> <a style='text-decoration:none; color:black;' href='index.php?edithm=".$row['id']."'>Sửa</a></button> || <button><a style='text-decoration:none; color:black;' href='index.php?Delhm=".$row['id']."' onclick='return confirm(\""."Bạn chắc có muốn xóa không"."\")'>Xóa</a></button>"."</td>";
